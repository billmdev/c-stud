...
int main (int argc, char** argv, char** envp) {
    ...
    // read terminal size and server connection data
    while (hsize == 0 || vsize == 0 || strlen(RHOST) == 0 || strlen(RPORT) == 0  ) {
			...
			// awaits for completion
				if (bg) {
					...
					if (bgp == 0) { // child connecting and executing to remote serve
						...
						continue;
				} else {  // foreground remote command, we execute it and wait for completion
